...
import 'skill_creation_assistant_store.dart';

class SkillCreationAssistantScreen extends StatefulWidget {
  final String connectionId;
  final String? agentId;

  const SkillCreationAssistantScreen({
    super.key,
  ...
  @override
  State<SkillCreationAssistantScreen> createState() => _SkillCreationAssistantScreenState();
}

class _SkillCreationAssistantScreenState extends State<SkillCreationAssistantScreen> {
  late final SkillCreationAssistantStore _store;
  ...
  void initState() {
    ...
    _skillsRepository = SkillsRepositoryImpl(chatRepository, widget.connectionId);

    _store = SkillCreationAssistantStore(
      _sessionRepository,
      chatRepository,
      _skillsRepository,
      connectionStore,
    ...
    // Watch for skill creation to show success state
    // Note: We don't auto-navigate here to avoid Navigator lock issues
    // during build phase. User can tap the skill name to navigate.
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(
      builder: (_) {
        final canPop = _store.skillCreated || !_store.hasSession || !_store.hasStartedConversation;
        ...
        return PopScope(
          ...
          child: Scaffold(
            appBar: AppBar(
              title: Text(localizations.translate('skill_create_title')),
            ),
  ...
  Widget _buildBody(BuildContext context, AppLocalizations localizations) {
    return Observer(
      builder: (_) {
        ...
        if (_store.errorMessage != null && !_store.skillCreated) {
          return ErrorState(
  ...
  Future<bool?> _showDiscardDialog(BuildContext context, AppLocalizations localizations) {
    ...
    return showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(
          localizations.translate('skill_create_discard_title'),
          style: AppTypography.headlineSmall(colorScheme.onSurface),
        ),
        content: Text(
          localizations.translate('skill_create_discard_message'),
          style: AppTypography.bodyMedium(colorScheme.onSurfaceVariant),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: Text(localizations.translate('skill_create_keep_editing')),
          ),
          FilledButton(
            ...
            child: Text(localizations.translate('skill_create_discard')),
          ),
        ],
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              localizations.translate('skill_create_success'),
              style: Theme.of(context).textTheme.headlineSmall,
              textAlign: TextAlign.center,
            ),
            Observer(
              builder: (_) {
                if (_store.createdSkillName != null) {
                  return Padding(
                    padding: const EdgeInsets.only(top: 8),
                    child: Text(
                      '"${_store.createdSkillName}"',
            ...
            FilledButton.icon(
              onPressed: _goBackToSkills,
              icon: const Icon(Icons.arrow_back),
              label: Text(localizations.translate('skill_create_back_to_skills')),
            ),
            ...
            OutlinedButton(
              ...
              child: Text(localizations.translate('skill_create_another')),
            ),
          ],
        ),
      ),
  ...
  void dispose() {
    ...
    // If the skill wasn't created and we have a session, delete it
    if (!_store.skillCreated && _store.hasSession) {
